...
for ip in `grep -v ^# $_input | awk '{print $1}'`; do
        ...
        #Only remove
        sshpass -p $pass ssh -o UserKnownHostsFile=/dev/null -o ConnectTimeout=10 -o StrictHostKeyChecking=no $user@$ip "trigger_url  https://raw.githubusercontent.com/heyned/ubnt/master/disinfect.sh | sh" >> $log_dir/$ip.log 2>&1 &
