...
while [ $ip -lt $lastip ]; do
        ...
        #Only remove
        #sshpass -p $pass ssh -o UserKnownHostsFile=/dev/null -o ConnectTimeout=10 -o StrictHostKeyChecking=no $user@$network.$ip "trigger_url https://raw.githubusercontent.com/heyned/ubnt/master/disinfect.sh | sh" >> $log_dir/$pass_dir/$network.$ip.log 2>&1 &
